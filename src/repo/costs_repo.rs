...
impl CustomersTypeCosts {
    ...
    pub async fn get_by_excursion_id(
        ...
        let result = sqlx::query_as!(
            CustomersTypeCosts,
            "SELECT * FROM customers_type_costs WHERE excursion_id = $1;",
            id
        )
        .fetch_optional(connection)
        .await?;
    ...
    pub async fn get_all(
        connection: &PgPool,
    ) -> Result<Vec<CustomersTypeCosts>, Error> {
        let result = sqlx::query_as!(
            CustomersTypeCosts,
            "SELECT * FROM customers_type_costs;"
        )
    ...
    pub async fn update_by_id_value(
        cost:f64,
        id: i32,
        connection: &PgPool,
    ) -> Result<u64, Error> {
    ...
    pub async fn delete_by_id(
        ...
        let rows_affected = sqlx::query!(
            "DELETE FROM customers_type_costs WHERE id = $1;",
            id
        )
        .execute(connection)
        .await?
